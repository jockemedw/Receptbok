...
### Senaste session
**Session 99 — Premiumvyn: vita knappar i helg-utfällningen:** Joakim: utfällningen av helg-receptkorten blir vit inom det annars sandfärgade kortet. Rotorsak: detaljpanelen (`.dlx-detail`) är genomskinlig och ärver kortets sandfärg korrekt — men de sekundära åtgärdsknapparna (`.dlx-act` / `.dlx-mini-btn`) hade `background: var(--paper)` (rent vitt). De är tänkta som *outline*-knappar (1.5px birch-soft-ram): på vardagskortets linnefärg (`#fdfcf8`) smälter det vita in, men på helgkortets sand (`#ede5d4`) poppar de som vita block. Fix: `background: transparent` på båda → knappen ärver kortets yta (linne på vardag, sand på helg) och definieras av ramen i båda fallen. Primärknappen (`Slumpa nytt`, rust) oförändrad. Bumpat `styles.css?v=112`, SW-cache v18. Live-verifiering kvar.
