...
- **Prisoptimering — reor-först-flöde (Session 121, ersatte den gamla toggeln):** Egen "Prisoptimera"-yta (knapp i Matsedel-rubriken, `js/weekly-plan/prisoptimera.js`), INTE längre en dold viktnings-toggle i genereringen. Steg 1: `GET /api/deals-offers` → Willys-reor **grupperade per ingrediens** (canon via `extractOfferCanon`), sorterade på bästa besparing; familjen kryssar i varor. Steg 2: `POST /api/deals-recipes {canons}` → recept som använder de valda varorna, **störst besparing först** (räknas bara på urvalet), via `matchRecipe`; "Lägg in på en dag" byter in i planen via `/api/replace-recipe`. Genereringen är numera prisagnostisk (gamla `optimize_prices`-toggeln borttagen; endpointens `optimize_prices`-kod finns kvar men matas inte längre av UI:t). Rör inte receptvals-/inköpsalgoritmerna.
