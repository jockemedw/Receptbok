...
- **Dagoperationer — EN modell (Session 142):** en dag i `meal_days` är en slot med innehåll (plandag, egen dag, notering, fri dag, tom) och alla typer flyttas/byts/kläms in likadant — inga pinnade fria dagar, inga plan-only-regler. Datumen ligger fast, innehållet byter datum. ALLT går via `api/day.js` (actions `swap`/`insert`/`push`/`pull`/`delete`) med rotationerna i `api/_shared/day-ops.js` (ett primitiv, invariant-signatur före skrivning, `fullContent` = allt utom datumet så inköpsrundstatus följer receptet). UI: dag-sheeten har *Flytta middagen* (flytta-läge; tom dag = flytta dit, upptagen = byt plats/kläm in), *Ingen middag* (push + notering, Ångra = pull), *Dra ihop*; drag & släpp är samma gest mot samma endpoint. Bygg aldrig en ny separat dagflytt-väg — utöka `day.js`.
