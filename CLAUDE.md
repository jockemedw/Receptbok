...
**Mjukare hero-snap (Joakim):** `snapToHero` gjorde ett hårt `window.scrollTo({top})` varje gång Matsedeln öppnades → kändes aggressivt + ingen animering. Nu mjuk glidning via `smoothScrollTo` (460 ms, eased) + hoppar över om vyn redan ligger ≈rätt (<24 px) så det inte rycker till. Global `scroll-behavior: smooth` undveks medvetet (skulle krocka med `smoothScrollTo`-loopen + scroll-återställning i `app.js`/`cook-mode.js`). Bumpat `app.js?v=113`, `plan-viewer-deluxe.js?v=113`. **Uppföljning:** Joakim ville fortfarande inte ha snap — den kvarvarande **CSS `scroll-snap-type: y proximity`** på `html` (+ `.dlx-hero` snap-align) ryckte tillbaka skärmen abrupt vid varje liten uppåtscroll. Helt borttagen → fri, mjuk native-rullning; man scrollar upp för att se historiken och stannar där. Positioneringen vid heron vid fliköppning sköts ändå mjukt av `snapToHero`. `styles.css?v=116`, SW-cache v25.

**Rationaliserade dagkorten (Joakim, bild på "salig blandning"):** egen-planering-dagar renderades i två stilar (valt recept = serif 1.08; fritext-not = sans 0.9) + en upprepad versal "EGEN PLANERING"-etikett. Fix: (1) etiketten borttagen → liten dämpad markör-ikon (`I.own`, `.dlx-own`) inline före rätten; (2) noteringar använder nu samma serif-titelstil (`.dlx-day-recipe`) som recept → en konsekvent rad oavsett typ; (3) tomma dagar visar en lugn plus-ikon (`I.plus`, `.dlx-add`) i stället för "+ Planera dagen"; (4) Ikväll-kortet använder samma markör i stället för "Egen planering"-undertexten. Gäller både list- och Ikväll-kort. Bumpat `app.js?v=114`, `plan-viewer-deluxe.js?v=114`, `styles.css?v=115`, SW-cache v24. **Uppföljning:** den enhetliga titeln gjorde att recept ur receptboken och egna noteringar såg identiska ut. Differentierade markör-ikonerna: recept-ur-boken = gryt-ikon i mjuk grön ton (`.dlx-own-recipe`), egen notering = neutral penna. `app.js?v=115`, `plan-viewer-deluxe.js?v=115`, `styles.css?v=117`, SW-cache v26. **Topp-omstrukturering + pull-to-reveal historik (Joakim):** (1) "Matsedeln" → "Matsedel" i rubrik, header-tab och bottom-nav. (2) `snapToHero` borttagen helt — sidan vilar nu på rubriken + heron under (switchTab scrollar redan till toppen). (3) Tidigare recept ligger inte längre i flödet ovanför heron; en **forcerad uppåtscroll i toppen** (touch rubber-band ~64 px / hjul-upp på desktop, `armHistory`) armar en snygg pill-knapp "Visa tidigare recept" som fäller ut historiken som ett **dragspel nedåt** (`_dlxHistoryArmed`/`_dlxHistoryOpen`, `dlxToggleHistory`) — utfällning nedåt vid toppen ger ingen scroll-ryck. Knappen göms igen om man scrollar ner utan att trycka, och historik-läget nollställs vid flikbyte. CSS scroll-snap är sedan tidigare borta. `app.js?v=116`, `plan-viewer-deluxe.js?v=116`, `styles.css?v=118`, SW-cache v27.
