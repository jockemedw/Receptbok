...
### Kända buggar
*(Inga öppna just nu — de två Session 100-buggarna åtgärdades Session 101, väntar på live-verifiering nedan.)*

### Väntar på live-verifiering (kod klar, ej körd skarpt)
- **Stapelvaru-dämpning i prisprio** (Session 101): `valueWeight()` (`api/_shared/willys-matcher.js`) dämpar nu billiga skafferi-stärkelser (`STAPLE_CANONS`: ris, pasta, nudlar, couscous, bulgur, farro, matvete + pastaformer) med `STAPLE_PENALTY = 0.3` så de aldrig lyfter ett recept över en proteinrea — prisvikten ensam räckte inte (1 kg-rispaket ~40 kr → vikt ~1.0). Visad kr-besparing oförändrad. Bekräfta mot produktion: prisoptimerad generering ger inte längre ris-/pasta-drivna förslag över kyckling/fetaost, varken i menyn (`bucketBySaving`) eller Veckans fynd.
- **Bekräfta/kassera-raden i premiumflödet** (Session 101): `#confirmPlanWrap` flyttas nu in som sista `.dlx-sec` i `#weekDeluxe` (efter dagslistan) via `relocateConfirmRow()` och tillbaka till klassisk position vid växling — samma DOM-nod, inga krockande id:n. Bekräfta på mobil: efter nygenererad plan visas bekräfta-/kassera-raden direkt under sista dagskortet i premiumvyn (inte som lös rad före ingredienssektionen), och växeln Premium↔Klassisk flyttar raden rätt i båda riktningarna.
...
### Senaste session
**Session 101 — Backlogg från Session 100: ris-rankning + bekräfta-raden i premiumvyn:** Åtgärdade de två kvarvarande Session 100-buggarna. (1) **Ris rankas över protein (prisprio):** `valueWeight()` (`api/_shared/willys-matcher.js`) viktade bara via erbjudandets ordinarie pris + protein-boost — men ett 1 kg-rispaket har pris ~40 kr → vikt ~1.0, alltså INGEN nedviktning (till skillnad från billig vitlök/lök som prisvikten redan trycker ner). Stora kr-besparingar på ris kunde därför lyfta recept över kyckling/fetaost. Fix: explicit stapelvaru-dämpning — `STAPLE_CANONS` (ris, risotto-ris, farro, bulgur, matvete, couscous, nudlar + pastaformer) × `STAPLE_PENALTY = 0.3` i `valueWeight`, motsatsen till protein-boosten. Slår igenom i både menyn (`bucketBySaving`→`weightedSaving`) och Veckans fynd. **Visad kr-besparing oförändrad** — bara rankningen. Medvetet *exkluderat* potatis (grönsak/huvudkomponent, ej skafferi-fyllnad) för att inte överstraffa potatisrätter. (2) **Bekräfta/kassera-raden i premiumvyn:** `#confirmPlanWrap` låg som lös sibling efter `#weekDeluxe` (hör hemma i klassiska `.week-section`) → hamnade visuellt skild från dagskorten. Ny `relocateConfirmRow()` (`plan-viewer-deluxe.js`) flyttar SAMMA DOM-nod in som sista `.dlx-sec` i `#weekDeluxe` (efter dagslistan) i premiumläge, och tillbaka till ursprungsläget (före `#weekRecipeDetail`) i klassisk vy — inga dubbletter, inga krockande id:n/handlers. Anropas i slutet av `renderDeluxe` + i `setMode` (för direkt växling). CSS: liten `margin-top` på confirm-sektionen. Tester: +5 assertions i `match.test.js` (141). Hela sviten grön (match 141, corpus 41, shopping 81, select 432, data-mapper 27, dispatch 93, cookies 29). Bumpat `styles.css?v=117`, `app.js?v=112`, SW-cache v24. Live-verifiering kvar — se Väntar-sektionen.
