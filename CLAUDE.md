...
- [~] 4D — Implementation **klar i kod, live-test kvar** (Session 38, pushad 2026-04-23). Endpoint `/api/dispatch-to-willys` (GET=featureAvailable, POST=runDispatch). Tre nya shared-moduler: `willys-search.js`, `willys-cart-client.js`, `dispatch-matcher.js`. Full plan: `docs/superpowers/plans/2026-04-23-willys-dispatch.md`. 53 nya assertions i `tests/dispatch-to-willys.test.js` bevakade av PostToolUse-hook.
- [~] 4E — Söknings-fallback **klar i kod tillsammans med 4D** (Session 38). Endpoint: `GET https://www.willys.se/search?q=<canon>`, publik. Canon-guard via befintlig `extractOfferCanon` + `CANON_REJECT_PATTERNS` löser vitlök→"Lök Vit"- och grädde→spraygrädde-buggar. 4E-rekon visade 18/20 rimliga första-träffar. Täckning förväntad: ~25% rea → ~75%+ med sök-fallback.
...
### Senaste session — Session 38 (2026-04-23) — Willys-dispatch implementation (Fas 4D+4E kod klar)
