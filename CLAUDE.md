...
- **"Flytta dag" + byt mot tom dag** (Session 86, PR #81): `api/move-day.js` (rotation av innehåll, fria dagar pinnade, simuleringsverifierad) + `api/swap-days.js` gap-gren (recept flyttas till tom dag, källdagen blir tom, plan-gränser räknas om). Aldrig körda mot live-DB. Bekräfta: (1) flytt mellan två dagar roterar rätt och bevarar fria dagar, (2) byt mot tom dag utanför planspannet uppdaterar start/slut, (3) inköpslistan orörd i båda fallen.
...
- **Efterfix (PR #79):** Ikväll-kortets datum flyttat till vänsterkolumn (`.dlx-day-when`/`dow`/`date` återanvänds) — samma layoutspråk som dagkorten. Eyebrow är åter bara "Ikväll".
