...
- **Status:** Kod live på main. Vercel deployar automatiskt. Knappen i Receptboken visar fortfarande env-baserad dispatch tills setup-stegen körts.
- **Nästa session (43):** Live-verifiera gist-flödet (öppna willys.se → kolla popup blir grön → klicka dispatch → verifiera "dispatch source=gist" i Vercel-logg), övervaka att två dispatchar går igenom, ta bort env vars som fallback. Sedan tillbaka till annan prioritering — kanske Fas 1F Priority 2-stemming live-test eller Fas 5A Capacitor-kickoff.
