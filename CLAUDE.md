...
### Öppna utredningar
**Avveckla klassiska veckovyn — full nedmontering (BESLUTAD Session 100, utförs nästa session):** Joakim har gått helt över till premiumvyn. Beslut: full nedmontering, inte bara dölj-toggeln. Kartläggning klar — `plan-viewer.js` (~1615 rader) innehåller tre sorters kod:
- **Klassisk-bara (radera):** tidslinje-kortgriden (`#weekGrid`-rendering i `renderWeeklyPlanData` ~rad 1049–), nav-chipsen (`timelineNav`), klassisk detaljpanel (`openWeekRecipe`), klassisk swap (`enterSwapMode`/`cancelSwapMode`/`swapDays`/`installSwapClickHandler`), `centerOnDate`/`centerTodayCard` (klassisk scroll), klassisk markup i `index.html` (`#timelineNav`, `#timelineOuter`/`#weekGrid`), och klassisk-bara CSS (`.week-day-card`, `.timeline-*`, nav-chips).
- **Delad data/logik (BEHÅLL):** `buildTimeline` → `window._timelineByDate` (premium läser den), `renderWeeklyPlanData` (data + state), `loadWeeklyPlan`, `loadArchive`/`loadCustomDays`.
- **Delad editor-panel (`#weekRecipeDetail`) — MIGRERA inline i premium, radera sedan panelen:** premium använder den idag via `openCustomDay` ("Redigera egen planering"), `openBlockedDay` (fri dag), plus replace-/custom-pick-flöden som byter till recept-fliken (`enterReplaceMode`/`selectRecipeForDay`, `enterCustomPickMode`/`selectRecipeForCustomDay`). Full nedmontering = flytta dessa editorer till premiumvyns inline-utfällning (likt `customDayEditorHtml` redan görs) och ta bort den delade bottenpanelen. **Detta är den riskfyllda delen** — rör delade flöden, så TDD/försiktigt. Hård regel: befintlig veckoplan får aldrig förstöras.
- **Toggeln:** ta bort `dlx-switch` + `setMode`/`applyMode`/`currentMode`/`STORAGE_KEY` i `plan-viewer-deluxe.js`; premium alltid på (ta bort `body.week-deluxe`-gatingen i CSS).
...
### Senaste session
**Session 100 — Byt dag för egna anteckningar + todo-hook:** Joakim: (1) lägg en hook som alltid startar en todo-lista vid sessionsstart, (2) gör det möjligt att byta plats på dagar man inte har recept på (egna anteckningar). **(1)** Ny andra SessionStart-hook i `.claude/settings.json` som echo:ar en TODO-påminnelse (aktiveras nästa session). **(2)** Egna anteckningar (egen planering) lagras som `meal_days`-rader med `plan_id = null`; de blockerades förut från "Byt dag" i premiumvyn (`modeCls`, `dlxCustomClick`, `dlxPickSwapTarget`) och i backend (`/api/swap-days` läste bara aktiva plan-rader och avvisade flytt *till* en custom-dag med 409). Fix — scope per Joakim: **full byt dag, bara premiumvyn** (klassiska vyn lämnad orörd, ska tas bort i separat steg). Backend `swap-days.js` omskriven: läser båda dagarna oavsett `plan_id`, byter *fullt* innehåll (inkl. `plan_id` + `custom_note`) i en atomär upsert när båda finns, annars flyttar raden via datumbyte (matchar nu på household+datum, inte `plan_id`, så custom-rader kan flyttas till tom dag); returnerar `customDays` i svaret. Premiumvyn: `modeCls` gör `custom` bytbar, "Byt dag"-knapp i custom-detaljen (not + custom-med-recept), `dlxCustomClick` väljer mål, `window._customDays` speglas från svaret. Invariant bevarad: ett recept byter bara dag (plan-receptmängd oförändrad → inköpslistan rörs ej). Bumpat `app.js?v=110`, SW-cache v21. Tester gröna (day-ops 34, match, corpus, shopping, select 432, data-mapper). Kvar: live-verifiering på mobil (se Väntar-sektionen). **Separat uppföljning:** ta bort klassiska vyn — premiumvyn återanvänder delade funktioner ur `plan-viewer.js` (`renderWeeklyPlanData`, `customDayEditorHtml`, `openCustomDay` m.fl.), så filen kan inte bara raderas; kräver egen refaktorering.

Session 8–99 i `docs/session-log-archive.md`. Full git-historik: `git log --oneline`.
