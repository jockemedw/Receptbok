...
- **Supabase free-tier pausar efter ~1 v inaktivitet** — hela appen (auth + receptdata + planer) ligger på Supabase gratistier. Efter ca en veckas total inaktivitet pausas databasen automatiskt; alla anrop failar tills någon väcker projektet (öppna appen/Supabase-dashboarden). Med daglig familjeanvändning inträffar detta i praktiken aldrig. Frontend visar begripligt svenskt "appen har vilat"-besked vid pausad DB (Session 106); kvarvarande lucka (backlog #11): ingen keep-alive-ping. Keep-alive via extern cron är OK enligt principerna (den *läser* bara en hälsosida, triggar ingen matsedel-generering).
- **Supabase-admin från Claude Code-miljön (riggat 2026-07-03):** Claude Code-miljön för repot har `SUPABASE_ACCESS_TOKEN` + `SUPABASE_PROJECT_REF` som miljövariabler och nätpolicy som tillåter `api.supabase.com`/`*.supabase.co`. Verifierings-SQL (läsningar mot `information_schema`/`pg_policies` etc.) får köras fritt via Management-API:t:
  ...
  **Schemaändringar (migrationer/DDL/skrivande SQL) körs BARA på Joakims uttryckliga uppmaning i sessionen** — aldrig autonomt i nattjobb. Arbetsgång: lägg idempotent SQL-fil i `db/migrations/` (`IF NOT EXISTS`/`OR REPLACE`), invänta klartecken, kör via API:t, verifiera. Joakim slipper SQL Editor-steget, men behåller grinden. Tokenen är kontobred — logga den aldrig, skriv den aldrig till repot.
...
- **Prisoptimering — reor-först-flöde (Session 121, ersatte den gamla toggeln):** Egen "Prisoptimera"-yta (`js/weekly-plan/prisoptimera.js`), INTE längre en dold viktnings-toggle i genereringen. Nås från **två ställen**: knappen i Matsedel-rubriken (`.prisopt-btn`) och en sekundär genväg i genereringsguidens steg 2 (`.wiz-prisopt` → `wizOpenPrisoptimera()`, som stänger guide-arket och öppnar Prisoptimera). Steg 1: `GET /api/deals` → Willys-reor **grupperade per ingrediens** (canon via `extractOfferCanon`), sorterade på bästa besparing; familjen kryssar i varor. Steg 2: `POST /api/deals {canons}` → recept som använder de valda varorna, **störst besparing först** (räknas bara på urvalet), via `matchRecipe`. Båda stegen bor i **en** serverless-function (`api/deals.js`, GET+POST i samma fil) — medvetet hopslaget för att hålla `api/` under Vercels **12-function-gräns på gratisplanen** (två separata filer tog katalogen till 13 och blockade hela deployen). "Lägg in på en dag" byter in i planen via `/api/replace-recipe`. Genereringen är numera prisagnostisk (gamla `optimize_prices`-toggeln borttagen; endpointens `optimize_prices`-kod finns kvar men matas inte längre av UI:t). Rör inte receptvals-/inköpsalgoritmerna.
- **Matchning (delad, oförändrad):** `matchRecipe`/`extractOfferCanon`/`canonProteinCategory`/`weightedSaving` i `willys-matcher.js` — samma canon-lexikon som inköpslistan. `bucketBySaving()`/`buildDealCandidates()`/"Veckans fynd"-popupen (`deals-popup.js`) finns kvar men matas inte längre av genereringen (ingen `optimize_prices` från UI:t); kan återanvändas/retireras vid behov.
