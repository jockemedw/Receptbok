...
const INGREDIENT_SIGNATURES = {
  italienskt:   [/parmesan/i, /pecorino/i, /mozzarella/i, /ricotta/i, /pancetta/i, /prosciutto/i, /balsamvinäger/i, /balsamico/i, /\bpest[oa]\b/i, /\bgremolata\b/i],
  mexikanskt:   [/\btortilla/i, /chipotle/i, /adobosås/i, /\bcotija\b/i, /\btomatill/i, /\bjalape[nñ]o/i, /majsmjöl.*tortilla/i],
  japanskt:     [/miso(pasta)?/i, /\bmirin\b/i, /\bdashi\b/i, /\bnori\b/i, /\bsake\b/i, /\bpanko\b/i, /\bedamame\b/i, /shiitake/i, /\bwasabi\b/i, /\bteriyaki\b/i],
  koreanskt:    [/\bkimchi\b/i, /gochujang/i, /\bgochugaru\b/i, /koreansk.*chili/i],
  indiskt:      [/garam masala/i, /\bgurkmeja\b/i, /\bkardemumma\b/i, /\bkurry/i, /\bcurrybl/i, /\bghee\b/i, /\bpaneer\b/i, /basmatiris/i, /\bmaserade\b/i, /\bpapadum\b/i, /tikka.*mas/i, /chana masala/i, /\btamarind/i],
  thailändskt:  [/fisksås/i, /citrongräs/i, /galangal/i, /thai.*basilika/i, /thaibasilika/i, /kaffirlim/i, /palmsocker/i, /röd.*curry.*pasta/i, /grön.*curry.*pasta/i, /\btom yum/i],
  vietnamesiskt:[/risnudl/i, /färska?\s+myntablad/i, /vietnam/i, /\bsriracha\b.*koriander/i],
  kinesiskt:    [/hoisin/i, /ostronsås/i, /shaoxing/i, /sichuan/i, /femkrydd/i],
  medelhavet:   [/\bfeta\b/i, /kalamataoliv/i, /tahini/i, /tzatziki/i, /\bkikärt/i, /halloumi/i, /\bfilo\b/i, /\bfilodeg\b/i],
  'mellanöstern':[/\bharissa\b/i, /za'atar/i, /zaatar/i, /\bsumak\b/i, /ras el hanout/i, /\bberber/i, /\bdukkah\b/i, /granatäppel(kärnor|sirap)/i],
  franskt:      [/dijonsenap/i, /herbes de provence/i, /vit.*vin.*cognac/i, /\bbeurre\b/i, /\bgruyère\b/i, /\bcr[eè]me fra[iî]che\b/i],
  asiatiskt:    [/sojasås/i, /sesamolja/i, /risvinäger/i, /sambal oelek/i, /\bingefära\b/i],
};
...
function classify(recipe) {
  ...
  // 90%+ regler:
  //   A) Titel-signatur (≥1 titel-träff) — distinkt namnord = säker
  //   B) ELLER ≥3 ingrediens-träffar i samma kök
  //   C) Plus: andraplatsen ska ha betydligt lägre score (≥10 marginal)
  const isTitleHit = topData.titleHits >= 1;
  const isStrongIngHit = topData.ingrHits >= 3;
  const margin = topData.score - (second?.[1].score || 0);
  const dominant = margin >= 10;

  if ((isTitleHit || isStrongIngHit) && dominant) {
