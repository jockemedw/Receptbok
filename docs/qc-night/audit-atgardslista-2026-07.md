...
- [x] **Säkra defensiva guards (PR #188):** F134 (extractRecipeCanons typeof-guard — stänger annars av prisopt för hela hushållet), F138 (alert res.ok + regressionstest), F116 (JSON-LD utan ingredienser → miss), F282 (array-medveten @type), F123 (fast quota-text, inget modellnamn-läck). Backend-only, svit grön.
- [ ] **Kvarvarande defensiva (låg prio, ej denna omgång):** F133 (BABY_FOOD_RE ytterligare åldersetiketter), F141 (proxy `this`, "funkar idag"), F118 (tags:[] tom-array-fallback), F119 (isForeignUrl mot slutlig redirect-URL).
