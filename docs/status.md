...
- **Verifieringskö (mobil, ej körd skarpt):** Session 102 nattjobb (svenska fel-toasts + död kod borttagen) · Session 101 nedmontering klassisk vy · Session 100 byt dag för egna anteckningar · Kontroll #2 dispatch rätt rea-vara.
- **Bygg näst:** #12 portionsskalning · #15 "Ikväll"-snabbredigerare · Hemköp parallell dispatch (PoC klar) · #3 atomär `activatePlan` · #5–7 Willys-cookies → Supabase/RLS.
- **Konfig som väntar på Joakim:** sätt `ALERT_WEBHOOK` (ntfy.sh) i Vercel → larm vid tyst Willys-degradering.
...
## Öppna utredningar
**App-analys-backlog (Session 102, `docs/app-analys-backlog.md`):** 18 av 27 punkter öppna efter nattjobbet. Prioriterade nästa steg: **#3 atomär `activatePlan`** (gör deactivate+activate till en Postgres-RPC — testet låser nu nuvarande icke-atomära beteende mot regressioner tills dess), **#5–#7** (Willys-cookies ur secret gist → Supabase+RLS, JWT-baserad household-härledning, multi-user dispatch — Fas 5-blockerare), **#10–#11** (custom-days-race, Supabase free-tier-paus). Produktspår: **#12 portionsskalning** + **#15 Ikväll-snabbredigerare** (agenternas två "bygg näst"). Hela listan med väg framåt i backlog-filen + sessionens todo.
