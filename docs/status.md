...
## Öppna utredningar
**Portionsvarianter — statiska förbakade versioner (design klar, bygg lokalt):** matlagningsläget ska byta från live-skalning (PR #106, ×0.5/×2) till **statiska portionsversioner 2/4/5/6/8** som Claude genererar granskat vid utveckling (inte runtime, inte självgående Gemini). Löser att mängder i instruktions-löptexten inte skalas idag. Fält-OK givet (`scaled` på receptet). Nästa session fortsätter på Joakims dator — **hela överlämningen + första steg: `docs/next-step-portionsvarianter.md`.**
